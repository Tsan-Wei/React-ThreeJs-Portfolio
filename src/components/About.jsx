...
const About = () => {
  return (
    <>
      ...
      我是一名全端工程師，擅長建置和維護高效、響應式的網站與應用程式。在前端技術方面，我熟練 <span className={styles.programingLanguage}>React.js</span> 和 <span className={styles.programingLanguage}>Vue.js</span> ，能創造吸引人的使用者介面。後端方面，我使用 <span className={styles.programingLanguage}>C# </span>與<span className={styles.programingLanguage}> ASP.NET MVC </span> 架構穩定的資料庫端邏輯。此外，我也有運用 <span className={styles.programingLanguage}> ThreeJS </span> 與<span className={styles.programingLanguage}> GSAP </span>等框架開發高互動性網站的經驗。我熱愛程式設計，喜歡探索新的技術，並致力於學習知識與利用科技解決生活的難題。
      </motion.p>
